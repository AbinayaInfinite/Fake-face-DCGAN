...
# Loading the dataset
dataset = dset.ImageFolder(root= './processed_celeba_small/celeba',transform = transform) # We use the images from ./processed_celeba_small/celeba folder and we apply the transformations on each image.
...
for epoch in range(25): # We iterate over 25 epochs.
    ...
    for i, data in enumerate(dataloader, 0): # We iterate over the images of the dataset.
        ...
        if i % 100 == 0: # Every 100 steps:
            vutils.save_image(real, '%s/real_samples.png' % "./processed_celeba_small/results", normalize = True) # We save the real images of the minibatch.
            fake = netG(noise) # We get our fake generated images.
            vutils.save_image(fake.data, '%s/fake_samples_epoch_%03d.png' % ("./processed_celeba_small/results", epoch), normalize = True) # We also save the fake generated images of the minibatch.
